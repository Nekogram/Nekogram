...
public class ReactionsContainerLayout extends FrameLayout implements NotificationCenter.NotificationCenterDelegate {
    ...
    public void setMessage(MessageObject message, TLRPC.ChatFull chatFull) {
        ...
        showExpandableReactions = (!premium || !allReactionsAvailable) && visibleReactions.size() > 16 || allReactionsAvailable && !UserConfig.getInstance(currentAccount).isPremium() && MessagesController.getInstance(currentAccount).premiumFeaturesBlocked();
